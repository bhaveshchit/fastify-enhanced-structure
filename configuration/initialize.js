...
function initialize(server) {
  server.addHook("onResponse", async (request, reply) => {
    ...
    const isRequestLog =
      projectConfig?.logs?.requestLog?.headers ||
      projectConfig?.logs?.requestLog?.params ||
      projectConfig?.logs?.requestLog?.query ||
      projectConfig?.logs?.requestLog?.body;
    beautifyResponseLogs(
      isRequestLog,
      `${JSON.stringify(request.method)} | ${JSON.stringify(request.url)}`,
      {
        statusCode: JSON.stringify(statusCode),
        responseTime,
        responseData: JSON.stringify(reply.responseData),
      },
    );
  });
}
...
(async () => {
  ...
  require("../dbConfig/mongoConnect");
  require("../utils/cron");
  require("../dbConfig/redis connected successfully");
})();
